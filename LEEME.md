...
* `-ua = Navegadores Aleatorios, por defecto: Avtivado`
...
Esto incluye, pero no se limita necesariamente a lo siguiente:

     Apache 1.xy 2.x
     dhttpd
     Websense "páginas bloqueadas" (sin confirmar)
     Trapeze Wireless Web Portal (sin confirmar)
     Módem de cable MI424-WR FIOS de Verizon (sin confirmar)
     Decodificador Motorola de Verizon (puerto 8082 y requiere autenticación - sin confirmar)
     BeeWare WAF (sin confirmar)
     Denegar todo WAF (parcheado) 
     Frasco (servidor de desarrollo)

Debido a que SlowJack explota los problemas para manejar miles de conexiones, el testeo tiene menos efecto en los servidores que manejan bien un gran número de conexiones. 

Se han recomendado servidores proxy y aceleradores de almacenamiento en caché como Varnish, nginx y Squid para mitigar este tipo particular de pruebas.

Además, ciertos servidores son más resistentes al testeo a través de su diseño, incluidos Hiawatha, IIS, lighttpd, Cherokee y Cisco CSS.
